...
class Mail {
  constructor() {
    this.transportConfig = {
      host: process.env.GCHECK_SMTP_HOST,
      port: process.env.GCHECK_SMTP_PORT
      // secure: process.env.SMTP_SECURE === 'true'
    };

    if (process.env.GCHECK_MTP_SECURE === 'true') {
      this.transportConfig.auth = {
        user: process.env.GCHECK_SMTP_AUTH_USER,
        pass: process.env.GCHECK_SMTP_AUTH_PASS
      };
    }

    if (process.env.GCHECK_SMTP_DEBUG === 'true') {
      this.transportConfig.debug = true;
    }
    if (process.env.GCHECK_SMTP_LOGGER === 'true') {
  ...
  async sendEmail({ from, to, subject, templatePath, templateValues } = {}) {
    ...
    fs.readFile(templatePath, { encoding: 'utf-8' }, (err, html) => {
      ...
      const sendData = {
        from: from || `"${process.env.GCHECK_EMAIL_FROM_NAME}" <${process.env.GCHECK_EMAIL_FROM_ADDRESS}>`,
        to,
        subject,
        html: renderedHtml
      };
  ...
  async sendVerificationEmail(to, name, verficationToken) {
    ...
    await this.sendEmail({
      ...
      templateValues: {
        first_name: name,
        verification_link: `${process.env.GCHECK_FRONTEND_URL}/register-confirm?key=${verficationToken}`
      }
    })
      .then(success => {
